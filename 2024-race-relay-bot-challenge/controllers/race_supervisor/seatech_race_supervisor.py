...
class SeatechRaceSupervisor(Supervisor):
    def __init__(self, test_mode=False):
        ...
        self.__teams_folder = abspath(join(dirname(abspath(__name__)), '../../teams'))
    ...
    def __fetch_teams(self):
        for team_name in listdir(self.__teams_folder):
            self.__teams.append(Team(team_name, join(self.__teams_folder, team_name)))
    ...
    def fetch_challengers_robots(self):
        ...
        for root, dirs, files in walk(self.__selected_team.repo_path):
            if 'worlds' in dirs and 'controllers' in dirs:
                print(root, dirs)
                challenger_world_file = join(root, 'worlds', '2024-simu_project.wbt')
                for controller_name in listdir(join(root, 'controllers')):
                    controller_path = join(root, 'controllers', controller_name)
                    ...
                    if exists(controller_path) and exists(challenger_world_file):
                        ...
                        # get robot Name
                        challenger.robot = str(proto.content)
                        # get robot Slots
    ...
    def pop_challenger(self, number):
        ...
        self.__selected_team.challengers[number].simu_DEF = self.getFromDef(node_name)
